...
export class ClassicController extends EntityController {
    private input: InputManager;
    private wasRightClickDown: boolean = false;
    ...
    public beforeRenderUpdate(): void {
        ...
        else if (this.input.inputMap[Key.RightClick] && !this.wasRightClickDown && !this.isInhaling) {
            this.playAnimation(Player.Animation.Inhale, false);
            this.isInhaling = true;
        }
        else if (this.input.inputMap[Key.RightClick] && this.wasRightClickDown && this.isAnimationPlaying(Player.Animation.Inhale)) {
            this.isInhaling = true;
        }
        else if (!this.input.inputMap[Key.RightClick] || !this.isAnimationPlaying(Player.Animation.Inhale)) {
        ...
        this.wasRightClickDown = this.input.inputMap[Key.RightClick];
        ...
        if (this.entity.position.x > 533.4 && this.entity.position.y < 22) {
            this.entity.dispose();
            Game.Instance.switchToGameOver(this.scene.score);
        }

        if (this.entity.position.y < -20) {
            this.entity.dispose();
            Game.Instance.switchToGameOver(this.scene.score);
        }
    }
